...
export function ListaHospitais() {
    const { hospitais, removerHospital, setHospitalEmEdicao } = useHospitais();
    ...
    const hospitaisFiltrados = hospitais.filter(
        (hospital) =>
            hospital.name.toLowerCase().includes(nomeFiltro.toLowerCase()) &&
            hospital.city.toLowerCase().includes(cidadeFiltro.toLowerCase()),
    );
                    ...
                    <div className='space-y-4'>
                        {hospitaisFiltrados.map((hospital) => (
                            <div
                                key={hospital.id}
                                onClick={() =>
                                    navigate(`/hospitais/${hospital.id}`)
                                }
