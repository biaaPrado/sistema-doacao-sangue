...
export function DoadorDetalhe() {
  ...
  const doador = doadores.find((d) => d.cpf === id);
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-md">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            <span className="px-4 py-1 rounded-full bg-red-100 text-red-700 font-semibold"> {doador.tipoSanguineo}{doador.fatorRh} </span>

            <div className="bg-red-50 border border-red-100 rounded-xl py-2 px-3">
              <p className="text-sm text-gray-500"> Total de Doações </p>
              <p className="text-xl text-right font-bold text-red-700"> {doador.historicoDoacoes?.length || 0} </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-gray-50 p-4 rounded-xl col-span-2">
            ...
            <p className="font-medium"> {formatarData(doador.dataNascimento)} </p>
          </div>
        </div>
