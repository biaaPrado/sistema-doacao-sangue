...
export function HistoricoDoacoes() {
    ...
    const doacoes = doadores
        .flatMap((doador) =>
            (doador.donations ?? []).map((doacao) => ({
                ...doacao,
                nomeDoador: doador.name,
                cpf: doador.cpf,
                tipoSanguineo: `${doador.bloodType.type}${doador.bloodType.rhFactor}`,
            })),
        )
        .filter((doacao) => {
            ...
            const tipoOk =
                filtroTipo === '' || doacao.tipoSanguineo === filtroTipo;

            const dataOk =
                filtroData === '' ||
                doacao.date.toISOString().split('T')[0] === filtroData;
                ...
                {doacoes.length === 0 ? (
                    ...
                    <div>
                        ...
                        {doacoes.map((doacao) => (
                            ...
                            >
                                <div className='p-3 text-center'>
                                    {doacao.date.toLocaleDateString('pt-BR')}
                                </div>
